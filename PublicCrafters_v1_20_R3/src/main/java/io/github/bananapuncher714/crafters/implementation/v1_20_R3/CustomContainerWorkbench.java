...
public class CustomContainerWorkbench extends ContainerWorkbench {
	...
	public CustomContainerWorkbench( int id, HumanEntity player, Location blockLocation, CustomInventoryCrafting crafting, InventoryCraftResult result ) {
		super( id, ( ( CraftHumanEntity ) player ).getHandle().fS() );
		...
		for ( int i = 0; i < 3; i++ ) {
			for (int j = 0; j < 9; j++) {
				a( new Slot( ( ( CraftHumanEntity ) player ).getHandle().fS(), j + i * 9 + 9, 8 + j * 18, 84 + i * 18 ) );
			}
		}
		for ( int i = 0; i < 9; i++ ) {
			a( new Slot( ( ( CraftHumanEntity ) player ).getHandle().fS(), i, 8 + i * 18, 142 ) );
		}
		
		try {
            Field titleField = Container.class.getDeclaredField( "title" );
            titleField.setAccessible( true );
            titleField.set( this, ContainerManager_v1_20_R3.WORKBENCH_TITLE );
            a( craftInventory );
	...
	public boolean a( RecipeHolder< ? extends IRecipe< InventoryCrafting > > holder ) {
		return holder.b().a( craftInventory, ( ( CraftHumanEntity ) viewer ).getHandle().dM() );
	}
